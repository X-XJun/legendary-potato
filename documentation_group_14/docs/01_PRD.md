...
Certificate PII (name, grades, etc.) stays off-chain in encrypted form referenced by IPFS CID; on-chain data is limited to wallet addresses, IPFS CIDs, and required credential lifecycle metadata (issuer, timestamps, expiry, revocation status/reason). The database stores:
